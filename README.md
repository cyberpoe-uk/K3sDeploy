...
Seeing a smaller root filesystem and a larger OS disk is not an error. For example, a Linux installer may place a 59 GiB root logical volume on a 120 GiB disk while the rest remains free inside the LVM volume group. That space is not visible to `parted` as unallocated disk space, so K3sDeploy checks both layers separately.

The same rule applies after a virtual disk is expanded. A VM can report a 220 GiB `/dev/sda` while `/` remains a 59 GiB logical volume. Shared-root storage is still unavailable because its guardrails apply to the mounted root filesystem, not the capacity of the disk beneath it. Choose separate OS-disk storage so K3sDeploy can inspect free LVM extents and unallocated space. The installer never grows the root filesystem automatically. This expanded-virtual-disk explanation is shown during installation only when K3sDeploy detects a virtual machine. Physical machines receive physical-disk guidance instead.
...
Every first-manager, manager-join, and worker-promotion workflow asks who will own embedded-etcd backups. The recommended managed policy enables K3s's native compressed snapshots independently on every manager. Each manager saves a local snapshot every 12 hours and retains only the five newest scheduled snapshots. This gives every surviving manager a local recovery point without unbounded scheduled-snapshot growth.

K3sDeploy also creates clearly named on-demand milestones after a successful first-manager installation, manager join, worker promotion, quorum recovery, or snapshot restore. Names include `k3sdeploy-first-manager-ready`, `k3sdeploy-manager-joined`, and `k3sdeploy-post-quorum-recovery`. K3s adds the manager name and timestamp to each file. The installer retains only the newest snapshot for each milestone name on each manager. This preserves distinct recovery points while preventing repeated runs from growing each milestone category forever. First-manager snapshots are created only after kube-vip, the selected load balancer, the selected storage controller, and health validation succeed. On a healthy manager installed by an older K3sDeploy release, option 6 offers a named baseline when no K3sDeploy milestone exists. K3sDeploy restricts the local snapshot directory to root and new milestone files to mode `0600`.

The advanced external-backup policy disables K3s scheduled snapshots and K3sDeploy milestone snapshots on that manager only after the operator types `MANAGE BACKUPS EXTERNALLY`. Existing snapshot files are not deleted. This option is intended for an operator with another verified backup process. A hypervisor snapshot can complement the process, but snapshots of different VMs taken at different times are not a coordinated embedded-etcd and application-data backup.

The normal local snapshot directory is `/var/lib/rancher/k3s/server/db/snapshots`. K3sDeploy reads `data-dir` or `etcd-snapshot-dir` from the managed K3s configuration when either value changes that location. Use `sudo k3s etcd-snapshot list` to inspect snapshots. One surviving manager can restore the control plane from its local snapshot when the matching server token is also available. A surviving worker cannot do this because workers do not store embedded etcd. If every manager or manager disk is lost, local snapshots are lost too. Copy important snapshots and `/var/lib/rancher/k3s/server/token` to encrypted off-node storage. K3s supports S3-compatible snapshot storage, but this release does not collect or store S3 credentials.
